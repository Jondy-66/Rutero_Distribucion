...
3.  **Gestión Diaria:**
    ...
    *   El cliente gestionado se marca como "Completado" y la aplicación presenta el siguiente cliente pendiente. Al finalizar el último, la ruta se marca como "Completada".
...
- **Reportes:** Los supervisores y administradores pueden acceder a una sección de reportes para visualizar y descargar en Excel la información de las rutas completadas por los vendedores de su equipo. Los usuarios pueden ver y descargar sus propios reportes.
...
- **Flujo**: El flujo de autenticación es gestionado por Firebase Authentication.
  ...
  4.  Al detectar un usuario, carga su perfil desde la colección `users` en Firestore y los datos globales (clientes, usuarios, etc.).
  5.  La aplicación redirige al `/dashboard`.

- **Roles de Usuario**: El sistema maneja cuatro roles: `Administrador`, `Supervisor`, `Usuario` y `Telemercaderista`. La lógica de autorización se implementa directamente en los componentes, mostrando u ocultando elementos de la interfaz según el `user.role` disponible en el `AuthContext`.

### 4.2. Gestión de Datos con Firestore

- **Modelo de Datos**: Los tipos principales (`User`, `Client`, `RoutePlan`, `Notification`, `PhoneContact`) están definidos en `src/lib/types.ts` y se corresponden con las colecciones en Firestore.
...
- **Optimización**: Para evitar exceder la cuota de Firestore, la aplicación sigue una estrategia de carga de datos optimizada:
  1.  **Carga Única**: Los datos de `users`, `clients`, `routes` y `phoneContacts`, que no cambian con extrema frecuencia, se cargan una sola vez al iniciar sesión en el `AuthProvider`.
